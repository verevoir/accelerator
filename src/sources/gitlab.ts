// GitLab source adapter — the SourceAdapter contract over GitLab's REST v4 API.
//
// Mirrors `@verevoir/sources/github` in shape and spirit: native fetch, no SDK,
// one place for headers + error mapping. It lives here until `@verevoir/sources`
// grows its planned `/gitlab` subpath; the contract is identical, so moving it
// upstream is a change of import path only.
//
...
// prompt-injected URL. HTTPS only, for the same reason.
//
...
/** Test hook: forget remembered default branches. */
 ...
 * GitLab requires a ref, so an omitted one resolves to the default branch. */
